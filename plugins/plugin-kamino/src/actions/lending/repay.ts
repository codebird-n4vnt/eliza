...
export const RepayAction: Action = {
    ...
    description: 'Repay borrowed debt in Kamino Lend. Use when the user wants to pay back a loan, reduce their debt, or close a borrowing position partially or fully.',
    validate: async (runtime: IAgentRuntime, message: Memory, state?: State): Promise<boolean> => {
        try {
            ...
            const obligation = await service.getUserObligation(market?.getName()!, ObligationTypeTag.Vanilla);
    ...
    examples:[
        [
            ...
            {name:'{{agent}}', content:{text:'Repayed 100 USDC from your loans...', actions:['KAMINO_REPAY']}},
        ],
        [
            {name:'{{user}}', content:{text:'Payback 20 SOL from my loans'}},
            {name:'{{agent}}', content:{text:'Payed back 20 SOL from your loans...', actions:['KAMINO_REPAY']}},
        ]
    ]
}