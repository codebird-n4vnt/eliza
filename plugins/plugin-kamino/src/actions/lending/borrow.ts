...
export const BorrowAction: Action = {
    ...
    handler: async (runtime: IAgentRuntime, message: Memory, state?: State, _options?: any, callback?: HandlerCallback) => {
        ...
        try {
            ...
            return { success: true, text: `Borrow success: Borrowed at ${apyLabel}% float` };
    ...
    examples: [
        [
            ...
            { name: '{{agent}}', content: { text: 'Fixed loans are not supported', actions: ['KAMINO_BORROW'] } },
        ],
        [
            { name: '{{user}}', content: { text: 'borrow 100 USDC' } },
            { name: '{{agent}}', content: { text: 'Borrowed 100 USDC at 8.23% variable.', actions: ['KAMINO_BORROW'] } }
        ],
        [
            { name: '{{user}}', content: { text: 'borrow 200 USDT at the cheapest rate' } },
            { name: '{{agent}}', content: { text: 'Borrowed 200 USDT at 7.40% variable.', actions: ['KAMINO_BORROW'] } }
        ]
    ]
