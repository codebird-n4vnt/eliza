...
export const KamninoLendDepositAction:Action =
{
  ...
  validate: async (runtime:IAgentRuntime,message:Memory):Promise<boolean> => {
    ...
    const hasDepositIntent = ['deposit', 'lend'].some((w)=> text.includes(w));
  ...
  handler: async (
    ...
    try {
      ...
      if(!parsed){
        const response = 'Please specify the token and amount. Example: "Deposit 100 USDC into Kamino"';
      ...
      const kaminoAction = await KaminoAction.buildDepositReserveLiquidityTxns({
        kaminoMarket: market!,
        amount: amountBase,
        reserveAddress: address(tokenAddress),
        owner: signer,
        obligation,
      ...
      logger.error('[KAMINO_LEND_DEPOSIT] Failed:', error);
