...
export const WithdrawAction: Action = {
    ...
    validate: async (runtime: IAgentRuntime, message: Memory, state?: State): Promise<boolean> => {
        try {
            ...
            const obligation = await service.getUserObligation(params?.marketName ?? 'main', ObligationTypeTag.Vanilla);
    ...
    examples: [
        [
            { name: '{{user}}', content: { text: 'Withdraw 100 USDC of my collateral' } },
            { name: '{{agent}}', content: { text: 'Withdraw of 100 USDC successfull...', actions: ['KAMINO_WITHDRAW'] } }
        ],
        [
            { name: '{{user}}', content: { text: 'Take out 20 SOL from my collateral' } },
            { name: '{{agent}}', content: { text: '20 SOL successfully taken out from your collaterals', actions: ['KAMINO_WITHDRAW'] } }
        ],
        [
            { name: '{{user}}', content: { text: 'Take out all of my SOL from collaterals' } },
            { name: '{{agent}}', content: { text: 'Succesfully taken out all SOL collateral', actions: ['KAMINO_WITHDRAW'] } }
        ]
    ],
}