...
export const HealthAction: Action = {
    ...
    handler: async (runtime:IAgentRuntime, message:Memory, state?:State, _options?:any, callback?:HandlerCallback)=>{
        try {
            ...
            const riskConfig:Record<string,{suggestion: string}> = {
                ...
                caution: {suggestion: 'Consider reducing borrows or adding collateral to improve your safety buffer.'},
                danger: {suggestion: 'Your position is at risk. Repay some debt or deposit collateral soon to avoid liquidation.'},
                critical: {suggestion: 'URGENT: You are close to liquidation. Repay debt or add collateral immediately.'}
            };
            ...
            for(const pos of health.positions) {
                ...
                lines.push(`Health Factor: ${pos.healthFactor} | LTV: ${pos.ltv}% | Net value: $${pos.netValue}`);
    ...
    examples:[
        [
            ...
            {name:'{{agent}}', content:{text:'Overall Risk: SAFE...', actions:['KAMINO_HEALTH']}}
        ],
        [
            {name:'{{user}}', content:{text:'Am I at risk of liquidation'}},
            {name:'{{agent}}', content:{text:'Overall Risk: CRITICAL - You are close to liquidation', actions:['KAMINO_HEALTH']}}
        ],
        [
            {name:'{{user}}', content:{text:'Show me my loans'}},
            {name:'{{agent}}', content:{text:'Overall Risk: CAUTION...', actions:['KAMINO_HEALTH']}},
        ],
    ],
}