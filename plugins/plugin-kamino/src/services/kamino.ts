...
import { KaminoMarket, PROGRAM_ID as KLEND_PROGRAM_ID, DEFAULT_RECENT_SLOT_DURATION_MS, type KaminoReserve, KaminoObligation, VanillaObligation, PROGRAM_ID, KaminoAction, parseKeypairFile, ObligationType, ObligationTypeTag, LendingObligation } from '@kamino-finance/klend-sdk';
...
export class KaminoService {
    ...
    async initialize(): Promise<void> {
        ...
        const marketsToLoad = settings.markets.length > 0 ? settings.markets : DEFAULT_MARKETS;
        ...
        for (const marketConfig of marketsToLoad) {
            try {
                ...
                if (!market) {
                    console.error(`[KaminoService] Market ${marketConfig.name} returned null from load()`);
                    continue;
                }
                ...
                console.error(`[KaminoService] Failed to load market ${marketConfig.name}:`, error);
            }
        }
    ...
    private getSettings(): KaminoPluginSettings {
        ...
        const rpcUrl = String(this.runtime.getSetting('KAMINO_RPC_URL')) || secrets.KAMINO_RPC_URL || 'https://api.mainnet-beta.solana.com';

        const privateKey = String(this.runtime.getSetting('KAMINO_PRIVATE_KEY')) || secrets.KAMINO_PRIVATE_KEY;

        const keypairPath = String(this.runtime.getSetting('KAMINO_KEYPAIR_PATH')) || secrets.KAMINO_KEYPAIR_PATH;
        ...
        const marketsJson = this.runtime.getSetting('KAMINO_MARKETS') || secrets.KAMINO_MARKETS;

        if (marketsJson) {
            try {
                markets = JSON.parse(marketsJson as string);
            } catch (error) {
                console.warn('[KaminoService] Failed to parse KAMINO_MARKETS, using defaults');
            }
        }
    ...
    private async loadSigner(settings: KaminoPluginSettings): Promise<KeyPairSigner<string> | null> {
        if (settings.keypairPath) {
            try {
                const resolvedPath = path.resolve(settings.keypairPath.startsWith('~') ? settings.keypairPath.replace('~', os.homedir()) : settings.keypairPath);
                return await parseKeypairFile(resolvedPath);
            } catch (error) {
                console.warn('[KaminoService] Failed to load keypair from file:', error);
            }
        }
        if (settings.privateKey) {
            try {
                ...
                console.warn('[KaminoService] failed to lead keypair from base58 string');
            }
        }
        ...
        this.refreshTimer = setInterval(async () => {
            for (const [name, market] of this.markets) {
                try {
                    ...
                    console.error(`[KaminoService] Reserve refresh failed for ${name} : ${error}`);
                }
            }
        }, intervalMs);
    }
    ...
    async getBoilerplate(
        ...
        const amountBn = new BN(amount.mul(new Decimal(10).pow(decimals)).toFixed(0));
    ...
    async getAllReserves(): Promise<ReserveInfo[]> {
        ...
        for (const [marketName, market] of this.markets) {
            const reserves = market.getReserves? market.getReserves() : [...market.reserves.values()];
            ...
            for (const reserve of reserves) {
                ...
                const availableLiquidityLamports = reserve.getLiquidityAvailableAmount();
                ...
                results.push({
                    ...
                    availableLiquidity: availableLiquidityLamports.div(mintFactor).toFixed(6),
    ...
    async getUserObligation(
        ...
        try {
            ...
            if(obligationTypeTag = ObligationTypeTag.Vanilla){
                obligation = await market.getUserVanillaObligation(this.getWalletAddress());
            } else {
                obligation = await market.getObligationByWallet(this.getWalletAddress(), new LendingObligation(this.getWalletAddress(), market.programId));
            }
    ...
    invalidateObligationCache(marketName: string, obligationTypeTag: ObligationTypeTag): void {
    ...
    async getAllUserObligations(forceRefresh = false): Promise<Map<string, KaminoObligation>> {
        const results = new Map<string, KaminoObligation>();

        for (const name of this.markets.keys()) {
            const obligation = await this.getUserObligation(name, ObligationTypeTag.Vanilla, forceRefresh);
    ...
    async getHealthCheck(forceRefresh = false): Promise<HealthCheckResult> {
        ...
        for (const [marketName, obligation] of obligations) {
            ...
            const healthFactor = stats.loanToValue.gt(0) ? obligation.liquidationLtv().div(stats.loanToValue) : new Decimal('999');
            ...
            for (const [reserveAddress, position] of obligation.borrows.entries()) {
                ...
                const borrowAPY = reserve ? reserve.totalBorrowAPY(currentSlot) : new Decimal(0);
    ...
    async sendActionTransaction(
        ...
        if (setupIxs.length > 0) {
            const {value: setupBlockhash} = await this.rpc.getLatestBlockhash({commitment:'finalized'}).send();
        ...
        const sendMainTx = async (): Promise<string> =>{
            const {value: latestBlockhash} = await this.rpc.getLatestBlockhash({commitment:'finalized'}).send();
